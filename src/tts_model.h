...
inline void copy_n(const ggml_tensor * first, size_t count, vector<float> & result) {
    result.clear();
    result.resize(count);
...
/// Readonly weights
struct tts_model {
    explicit tts_model(const shared_ptr<gpu_context> & gpu, const model_tensor_meta & tensor_meta);
...
protected:
    const ggml_tensor * copy_to_gpu(const ggml_tensor * src);
