...
struct parler_tts_model : tts_model {
    ...
    explicit parler_tts_model(gguf_context * meta_ctx, ggml_context * load_context, bool cpu_only, bool use_cross_attn_) : use_cross_attn{use_cross_attn_} {
...
void assign_parler_layer(parler_tts_model * model, parler_layer & layer, string name, ggml_tensor * tensor);
void assign_to_decoder(parler_tts_model * model, const string name, ggml_tensor * tensor);
...
ggml_tensor * parler_build_inp_embd(struct ggml_context * ctx, struct parler_context * pctx, parler_tts_model * model, const parler_ubatch & batch);
ggml_tensor * parler_build_layer_norm(struct ggml_context * ctx, ggml_tensor * inputs, ggml_tensor * weight, ggml_tensor * bias);
void parler_build_kv_store(struct ggml_context * ctx, const parler_kv_cache * kv, struct ggml_cgraph * graph, ggml_tensor * k_cur, ggml_tensor * v_cur, int32_t n_tokens, int32_t kv_head, int32_t index, int32_t n_embd_gqa);
ggml_tensor * parler_build_head_outputs(struct ggml_context * ctx, parler_tts_model * model, ggml_tensor * cur);
ggml_tensor * build_attn_mask(ggml_context * ctx, parler_context * pctx, parler_ubatch & batch);
ggml_tensor * build_attn_mask_cross(ggml_context * ctx, parler_context * pctx, parler_tts_model * model, parler_ubatch & batch);
static struct parler_ubatch batch_from_sentence(string sentence, parler_tts_model * model, unigram_tokenizer * tokenizer);
...
struct parler_tts_runner : tts_runner_with_dac {
    parler_tts_runner(parler_tts_model * model, dac_runner * audio_decoder, parler_context * pctx, unigram_tokenizer * ut): model(model), dac_runner(audio_decoder), pctx(pctx), tokenizer(ut){};
    ~parler_tts_runner() {
        if (ctx) {
            ggml_free(ctx);
        }
    ...
    int generate(string sentence, struct tts_response * response, int32_t seq_id = -1) override;
    bool check_stopping();
    ...
    void update_conditional_prompt(str file_path, str prompt, int n_threads, bool cpu_only = true) override;
};
