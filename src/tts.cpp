...
static bool dia_is_quantizable(sv name, const quantization_params & params) {
    ...
    return !name.starts_with("audio_encoder") && !name.ends_with("norm") &&
           (params.quantize_output_heads || !name.ends_with("dia.decoder.heads"));
}

bool parler_is_quanitizable(sv name, const quantization_params & params) {
    ...
    return !name.ends_with("audio_encoder") && !name.ends_with("norm.weight") && !name.ends_with("text_encoding") &&
