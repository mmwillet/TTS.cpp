...
static ggml_tensor * build_lstm_run(ggml_context * ctx, ggml_tensor * input, ggml_tensor * h_0, ggml_tensor * c_0, std::vector<ggml_tensor*> weights, std::vector<ggml_tensor*> biases, uint32_t sequence_length, bool reversed) {
...
static ggml_tensor * build_ada_residual_conv(ggml_context * ctx, ggml_tensor * x, ada_residual_conv_block * block, ggml_tensor * style, ggml_tensor * sqrt_tensor) {
    ...
    ggml_tensor * gamma = ggml_add(ctx, ggml_mul_mat(ctx, block->norm1_gamma, style), block->norm1_gamma_bias);
    ggml_tensor * beta = ggml_add(ctx, ggml_mul_mat(ctx, block->norm1_beta, style), block->norm1_beta_bias);
...
static ggml_tensor * build_kokoro_generator_res_block(ggml_context * ctx, ggml_tensor * x, ggml_tensor * style, kokoro_generator_residual_block * block) {
    ...
    for (int i = 0; i < block->convs1_weights.size(); i++) {
        ggml_tensor * gamma = ggml_add(ctx, ggml_mul_mat(ctx, block->adain1d_1_gamma_weights[i], style),
                                       block->adain1d_1_gamma_biases[i]);
        ggml_tensor * beta = ggml_add(ctx, ggml_mul_mat(ctx, block->adain1d_1_beta_weights[i], style),
                                      block->adain1d_1_beta_biases[i]);
        ggml_tensor * cur = ggml_cont(ctx, ggml_transpose(ctx, ggml_norm(ctx, inpl, 0.00001)));
...
int kokoro_runner::generate(std::string prompt, tts_response * response) {
...
kokoro_context * build_new_kokoro_context(kokoro_model * model, int n_threads, bool use_cpu) {
    ...
    kctx->set_threads();
    kctx->build_schedule();
    // TODO preserve these multipliers: 30 vs 5
