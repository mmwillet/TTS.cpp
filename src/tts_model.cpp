...
gpu_context::gpu_context(int n_threads, bool cpu_only)
    : cpu{ggml_backend_cpu_init()}, _gpu{
          ...
          cpu_only ? nullptr : ggml_backend_metal_init()
#else
          nullptr
#endif
      }, threadpool{
...
tts_model::tts_model(const shared_ptr<gpu_context> & gpu, const model_tensor_meta & tensor_meta)
    : ctx{
          ggml_init({
...
const ggml_tensor * tts_model::copy_to_gpu(const ggml_tensor * src) {
    ...
    // TODO: upstream const src to ggml-backend.cpp:ggml_backend_tensor_copy
    ggml_backend_tensor_copy(const_cast<ggml_tensor *>(src), result);
    return result;
}
